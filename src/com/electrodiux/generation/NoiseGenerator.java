...
public class NoiseGenerator {
    ...
    public NoiseGenerator(long seed) {
        this(generatePermutation(seed), 1, 2, 0.5f);
    }
     ...
     *                          result
     */
    public NoiseGenerator(long seed, int octaves, float lacunarityFactor, float persistanceFactor) {
        this(generatePermutation(seed), octaves, lacunarityFactor, persistanceFactor);
    }

    private NoiseGenerator(int[] perm, int octaves, float lacunarityFactor, float persistanceFactor) {
    ...
    public float[] getNoise2D(int width, int height, int posX, int posY) {
        return getNoise2D(width, height, posX, posY, 1, 8, octaves, lacunarityFactor, persistanceFactor);
    }

    public float[] getNoise2D(int width, int height, int posX, int posY, int range) {
        return getNoise2D(width, height, posX, posY, range, 8, octaves, lacunarityFactor, persistanceFactor);
    }
    ...
    public float[] getNoise2D(int width, int height, int posX, int posY, float range, int chunkSize, int octaves,
        ...
        for (int oct = 0; oct < octaves; oct++) {
            float noiseScale = lacunarity / chunkSize;
    ...
    private double noise(double x, double y, float noiseScale) {
        x *= noiseScale;
        y *= noiseScale;
     ...
     */
    public float[] getNoise3D(int width, int height, int depth, int posX, int posY, int posZ,
            float range, int chunkSize, int octaves, float lacunarityFactor, float persistanceFactor) {
        ...
        for (int oct = 0; oct < octaves; oct++) {
            ...
            for (int i = 0; i < width * height * depth; i++) {
                ...
                double noise = noise3D[x + y * width + z * width * height]
                        + noise(x + posX, y + posY, z + posZ, noiseScale) * persistance;
                if (oct + 1 == octaves) {
    ...
    private double noise(double x, double y, double z, float noiseScale) {
        x *= noiseScale;
        y *= noiseScale;
        z *= noiseScale;
